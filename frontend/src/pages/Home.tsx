...
export function Home(_props: Props) {
  ...
  const effectiveProjects = useMemo(() => {
    ...
    return managedProjects.map((managed, index) => {
      ...
      return project
        ? { ...project, title: managed.title, titleEn: managed.title, image: managed.cover || project.image }
        : { id: 1000 + index, slug: managed.slug, title: managed.title, titleEn: managed.title, image: managed.cover }
    })
  }, [managedProjects])
      ...
      <section className="home-perspective" aria-labelledby="product-perspective-title">
        ...
        <div className="home-perspective-list">
          <article>
            <span>01</span>
            <div>
              <h3>{locale === 'zh' ? '先理解问题，再表达答案' : 'Understand the problem before expressing the answer'}</h3>
              <p>{locale === 'zh' ? '好的产品设计从真实情境出发。先看清用户、业务与技术之间的约束，再决定界面应该呈现什么，而不是从一个漂亮的页面开始。' : 'Good product design begins with real context. Understand the constraints between users, business, and technology before deciding what the interface should show.'}</p>
            </div>
          </article>
          <article>
            <span>02</span>
            <div>
              <h3>{locale === 'zh' ? '复杂留给系统，清晰交给用户' : 'Keep complexity in the system and clarity with the user'}</h3>
              <p>{locale === 'zh' ? '设计的价值不是隐藏所有复杂度，而是建立恰当的信息层级与反馈，让用户始终知道发生了什么、为什么发生，以及下一步可以做什么。' : 'Design does not hide all complexity. It creates the right hierarchy and feedback so people know what happened, why it happened, and what they can do next.'}</p>
            </div>
          </article>
          <article>
            <span>03</span>
            <div>
              <h3>{locale === 'zh' ? '把设计当作持续演进的系统' : 'Treat design as an evolving system'}</h3>
              <p>{locale === 'zh' ? '产品不会在上线时完成。通过真实使用、数据与反馈持续校准，让每一次迭代既解决当下问题，也为下一次变化留下空间。' : 'A product is not finished at launch. Real usage, data, and feedback should keep shaping it while every iteration leaves room for what comes next.'}</p>
            </div>
          </article>
        </div>
      </section>
